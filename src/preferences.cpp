...
Preferences::Preferences(Tab* t)
{
	...
	connect(_listWidget, SIGNAL(currentRowChanged(int)), _stackedWidget, SLOT(setCurrentIndex(int)));
...
void Preferences::revert()
{
	if(_listWidget->currentRow() == 0){
		_colorChanges = Atom::labelToColor;
		if(_currentTab != NULL)
			_currentTab->canvas->updateAtomColors(_colorChanges);
	}
}
...
{
	AtomButton* a = new AtomButton(_currentTab, label);
